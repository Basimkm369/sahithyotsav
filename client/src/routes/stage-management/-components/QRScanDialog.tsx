...
interface QRScanDialogProps {
  open: boolean;
  onClose: () => void;
  participants: { chestNumber: string; status: string;[key: string]: any }[];
  onValidScan: (participant: any) => void;
}

const QRScanDialog: React.FC<QRScanDialogProps> = ({ open, onClose, participants, onValidScan }) => {
        ...
        <div className="space-y-4">
          ...
          <div className="relative border rounded-md overflow-hidden h-64 bg-gray-100 flex items-center justify-center">
            {selectedCamera ? (
              ...
              <span className="text-muted-foreground">Select a camera to start scanning</span>
            )}
          </div>
