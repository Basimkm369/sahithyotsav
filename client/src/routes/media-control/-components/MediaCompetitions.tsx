import useMediaCompetitions, {
  Competition,
} from '../-hooks/useMediaCompetitions'
import {
  Select,
  SelectTrigger,
  SelectValue,
...
import MediaCompetitionCard from './MediaCompetitionCard'
import MediaCompetitionCardSkeleton from './MediaCompetitionCardSkeleton'
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading
            ? Array.from({ length: 24 }).map((_, i) => (
                <MediaCompetitionCardSkeleton key={i} />
              ))
            : data?.map((comp) => (
                <MediaCompetitionCard
                  data={comp}
                  key={comp.itemCode}
