...
import useAnnouncementCompetitionDetails, {
  CompetitionDetails,
} from '../-hooks/useAnnouncementCompetitionDetails'
import { Route } from '..'
...
export default function AnnouncementCompetitionModal({
  data: competition,
  open,
  onClose,
}: {
  ...
  const { data, isLoading } = useAnnouncementCompetitionDetails({
    eventId,
  ...
  const [component, promptComfirmation] = useConfirmation({
    ...
    onConfirm: async (status) => {
      ...
      await api.post(`/announcementControl/updateCompetitionStatus`, {
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                    {competition.status === CompetitionStatus.Finalized && (
                      <Button onClick={() => promptComfirmation(CompetitionStatus.Announced)}>
                        Mark as Announcement Completed
                      </Button>
                    )}
